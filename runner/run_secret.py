...
IMAGE = "twc:mvp"
OUT_CSV = "data/secret_dataset.csv"
...
def read_kv_file(path: Path) -> dict[str, int]:
    # Treat ENODEV / teardown races as "missing"
    try:
        text = path.read_text()
...
def run_one(secret_N: int, rep: int, size_mib: int = 128, hold_ms: int = 2000, poll_interval_s: float = 0.1) -> dict:
    run_id = str(uuid.uuid4())
    name = f"twc_secret_{secret_N}_{rep}_{run_id[:8]}"
...
def main():
    ...
    for n in secret_levels:
        for rep in range(reps_per_level):
            schedule.append((n, rep))

    random.seed(2026)
    random.shuffle(schedule)

    # overwrite file for a clean run
    if os.path.exists(OUT_CSV):
        os.remove(OUT_CSV)

    for (n, rep) in schedule:
        row = run_one(n, rep, size_mib=size_mib)

        print(
            f"secret N={n} rep={rep:02d} "
